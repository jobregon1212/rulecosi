...
data = pd.read_csv('data\ecoli0vs1.csv')
...
for k_fold, (train_idx, test_idx) in enumerate(cv_exps.split(X, y)):
    if test_fold == test_fold:#k_fold:
        ...
        rule_clf = RuleCOSIClassifier(base_ensemble=LGBMClassifier(n_estimators=10, random_state=random_state), random_state=1212, max_rule_depth=10, rule_order='conf')
